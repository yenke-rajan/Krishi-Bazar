...
export default function WholesalerPlaceOrder() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      setSuccessOrder(await res.json());
  ...
  if (successOrder) {
    return (
      <div className="flex flex-col items-center py-10 gap-4">
        ...
        <button
          onClick={() => setLocation('/wholesaler/my-orders')}
    ...
    <>
      ...
      {step === 1 && (
        <div>
            ...
            {(['ALL', 'VEGETABLE', 'PICKLE'] as CategoryFilter[]).map((cat) => (
              <button key={cat} onClick={() => setCategoryFilter(cat)}
                className={['px-3 py-1.5 rounded-full text-[12px] font-semibold whitespace-nowrap border',
                  categoryFilter === cat ? 'bg-kb-marigold text-white border-kb-marigold' : 'bg-white text-kb-muted border-kb-border'].join(' ')}>
          ...
          <button onClick={() => setStep(2)} disabled={!selectedCrop}
      ...
      {step === 2 && (
        <div>
          ...
          <div className="relative">
            <input type="number" min={1} max={10000} value={weightKg}
